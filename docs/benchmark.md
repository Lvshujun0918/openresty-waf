...
- **请求级变量提取缓存**：原实现中每条规则独立解析其声明变量，导致多规则
  重复解析 URI_ARGS / HEADERS / BODY。优化后同一请求内相同变量只解析一次
  （`variables.collect(var, ctx)` 按 类型+specific+parse 缓存于请求上下文），
  多规则共享提取结果。
- **正则说明**：OpenResty 新版已移除 `ngx.re.compile`（re.lua 仅含 split/opt），
  正则统一走 `ngx.re.find(..., "joi")`，依赖 PCRE JIT 与 compile-once 缓存。

### 优化前后对比（普通请求）

| 场景 | RPS | 平均延迟 | 相对基准 |
|---|---|---|---|
| 基准（无 WAF） | 179,211 | 0.558 ms | 100% |
| 优化前 加 WAF | 93,789 | 1.066 ms | -47.7% |
| **优化后 加 WAF** | **107,613** | **0.929 ms** | **-39.9%** |

- 吞吐提升 **+14.7%**，平均延迟降低 **-12.9%**。
- 相对无 WAF 基准的损耗从 -47.7% 收窄至 -39.9%。

### 后续可继续的优化方向

- **检测剪枝**：静态资源后缀 / 白名单 URL 直接放行（跳过规则引擎）。
- **命中缓存**：同 IP+URI 短时缓存判定结果。
- **日志异步化**：生产切 redis 后端，消除攻击场景文件 IO 瓶颈（实测攻击场景
  额外 ~18pct 开销主要来自日志写盘）。
