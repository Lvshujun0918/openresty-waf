...
-- 错误汇总上报
-- ============================================================================
-- 引擎 ERR/WARN 级错误异步上报 Redis 队列，后台消费后在「报错汇总」页统一展示；
-- worker 本地限速 + 同签名去重防洪，Redis 不可用时静默不影响业务
_M.errlog = {
    enabled      = true,
    redis_key    = "waf:error:list", -- 后台消费队列
    max_per_min  = 60,               -- 每 worker 每分钟最多上报条数
    dedup_window = 10,               -- 相同错误签名去重窗口（秒）
}
...
_M.upload = {
    ...
    -- 内容特征扫描窗口：图片马/Webshell 探测读取文件头尾各半的字节上限
    -- （默认 1MB：头部 + 尾部各 512KB 采样；文件小于该值时全量扫描）
    content_scan_limit = 1048576,
}
