...
local function build_xdb(segs)
    ...
    local VEC = 256 * 256
    local header_len = 16
    local seg_start = header_len + VEC * 8
    local n = #segs
    local segs_buf, data_buf, dsum = {}, {}, 0
    for i, sg in ipairs(segs) do
        local dptr = seg_start + n * 14 + dsum
        segs_buf[i] = le32(ip2u(sg[1])) .. le32(ip2u(sg[2])) .. le16(#sg[3]) .. le32(dptr)
        data_buf[i] = sg[3]
        dsum = dsum + #sg[3]
    end
    ...
    local header = le16(2) .. le16(1) .. le32(0) .. le32(VEC * 8) .. le32(0)  -- version=2, 16B
    return header .. table.concat(vec) .. table.concat(segs_buf) .. table.concat(data_buf)
end

local tmp = "/tmp/test_ip2region.xdb"
local xdb = build_xdb({
    { "8.8.8.0",   "8.8.8.255",   "美国|0|0|Level3|US" },
    { "1.0.0.0",   "1.0.0.255",   "中国|北京|北京|电信|CN" },
    { "223.5.5.0", "223.5.5.255", "中国|浙江|杭州|阿里云|CN" },
})
